...
def comparison(expected_array, actual_array, combine):
	...
	if combine:
		...
		bin_count = int(np.ceil((combined.max() - combined.min()) / bin_width))
	else:
		q1 = np.quantile(expected_array, 0.25)
		q3 = np.quantile(expected_array, 0.75)
		iqr = q3 - q1
		bin_width = (2 * iqr) / (len(expected_array) ** (1 / 3))
		bin_count = int(np.ceil((expected_array.max() - expected_array.min()) / bin_width))
...
def runMain(gt, pred, outFileFormat, combineLabels, outDir):

	df_gt = pd.read_csv(str(Path(gt)/'Feature_Extraction.csv'))
	df_pred = pd.read_csv(str(Path(pred)/'Feature_Extraction.csv'))

	header = ['features','histogram intersection','correlation', 'chi square', 'bhattacharya distance','L1 score','L2 score', 'L infinity score','cosine distance','canberra distance','ks divergence','match distance','cvm distance',\
	...
	for feature in df_gt.columns[1:]:
		...
		if z_pred.size > 3 and z_gt.size > 3:
			...
			psi_value, kld_value, jsd_value, wd_value, p_value, errors = comparison(z_gt, z_pred, combineLabels)

			data_result = [feature, hist_intersect,correlation, chi_square, bhattacharya,l1,l2, linfinity,cosine_d,canberra,ks_divergence,match,cvm,\
...
# combineLabels = False; outFileFormat = True
# file_gt = 'C:/Users/goyalv2/Downloads/gt_features/'
# file_pred = 'C:/Users/goyalv2/Downloads/pred_features/'
# outDir = "C:/Imaging/benchmarking/output"
# runMain(file_gt, file_pred, outFileFormat, combineLabels, outDir)